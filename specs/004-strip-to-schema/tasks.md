...
- [X] T035 [US3] Record the disposition of #35 and #36 as a comment on each, and let the merge close them — their own linkage rule says an issue is closed when its fix is on `main`, and these commits are on a branch (FR-021): both are argued against `ARCHITECTURE.md`, `LAYOUT.md` and documents that no longer exist, and 3.0.0 removes the ADR requirement that made the ladder's retirement owe one. Same for #39 and #43 — the claim moved and was restated (T009), and the workaround's cause disappeared with `schema/README.md` (T008), though #43's underlying defect is not fixed and is argued to stay open. Same for #40, #41 and #45, superseded by this feature; #44 is the branch's closing link
- [X] T036 [US3] Add a comment to #46 recording that `.copier-answers.yml` would now regenerate `AGENTS.md` against a tree two cuts old, and that its own header forbids hand-editing, so the fix is to re-answer. Leave it open. Leave #37, #38 and #42 open — all are schema or gate defects this feature does not touch
- [X] T037 [US3] Record, in the pull request body, what each removed document was for and why it is gone (FR-020), so a later decision to rebuild any of it starts from the argument rather than from `git log`. The disposition table in [data-model.md](data-model.md) is the source

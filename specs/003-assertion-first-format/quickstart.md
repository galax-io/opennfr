...
```bash
python3 scripts/opennfr_check.py 'conformance/**/*.yaml'
```
...
python3 scripts/opennfr_check.py /tmp/probe.yaml
```
...
```bash
python3 scripts/opennfr_check.py --render-cases 'conformance/render/*'
```
...
```bash
bash scripts/verify.sh   # after planting one defect by hand
```

**Not yet automated.** The mutation suite is specified in
[contracts/corpus-runner.md](contracts/corpus-runner.md) § E4 and not written; today each
defect is planted by hand and the build must go red. Expected outcomes fall in two groups:
...
```bash
PATH=/usr/bin:/bin bash scripts/verify.sh   # both libraries absent
```

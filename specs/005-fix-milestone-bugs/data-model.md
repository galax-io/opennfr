...
| `schema/opennfr.io/v1/requirementset.schema.json` → root and `$defs/{name,unit,op,aggregation,annotations,displayName}` | R6 (#31) | Gain `examples`; `predicate`, `requirement` and `selector` already had them | Every constraint — this adds documentation, not validation |
| `scripts/verify.sh` — the examples half of the self-check | R6 (#31) | The required set is derived from `$defs` instead of a hand-kept list, and the root example is validated | The closure probes |
